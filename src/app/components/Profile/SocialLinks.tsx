...
const SocialLinks: FC<SocialLinksProps> = ({ links }) => {
  ...
  return (
    <div className="mt-4 grid grid-cols-1 gap-3 print:gap-1 print:grid-cols-1 sm:grid-cols-2 md:grid-cols-1">
      {links.map((e) => {
        const Icon = SOCIAL_LINK_ICON_MAP[e.name];
        return (
          <a
            key={e.name}
            className="group flex items-center gap-3 text-slate-600 no-underline transition-all duration-200 hover:text-primary-600 print:text-slate-900 dark:text-slate-400 dark:hover:text-primary-400"
            href={e.link}
            target="_blank"
